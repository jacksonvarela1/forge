const DAYMETA = {
  mon:{abbr:'MON',weapons:'all',title:'Kicks & Hips',type:'technical',intensity:3,warm:'hips',cool:'hips',lift:'Lift: Upper A + forearms.',focus:'The only leg-skill day of the week. Ladder into it, do not open at full height.',flag:'Two days after Saturday\u2019s squat and the legs are still coming back, and this morning\u2019s pogos already taxed the calves. Low kicks, then body, then head, and if a head kick is on today\u2019s list leave it until last. The base foot is the limiter today, not the kicking leg.'},
  tue:{abbr:'TUE',weapons:'all',title:'Combinations',type:'technical',intensity:3,warm:'upperk',cool:'upper',lift:'No lift today. This is one of only two low days you get.',focus:'Chain it clean at 60 to 70 percent. Hands lead, the legs just carry you.',flag:'One of only two low days you get, and you lift again tomorrow. Hands-led is what keeps this cheap: no fight-pace finishes, no sprint rounds, nothing that leaves the legs heavy. Walk away with air left.'},
  wed:{abbr:'WED',weapons:'hands',title:'Boxing',type:'technical',intensity:2,warm:'upper',cool:'upper',lift:'Lift: Lower Hamstring + abs.',focus:'Hands and angles. The richest technical session of the week, and physically a coast.',flag:'You hinged heavy this morning, so the feet step and pivot tonight, they never bounce. Nothing that loads the legs: no rope, no skipping, no bounding. Hands and angles is the whole session.'},
  thu:{abbr:'THU',weapons:'hands',title:'Output & Core',type:'hard',intensity:5,warm:'full',cool:'full',lift:'No lift today. The only clean hard day in the week.',focus:'The one session where you chase pace. Hands, trunk, breathing.',flag:'24 hours after RDLs and Nordics, which is when hamstring soreness peaks. Output comes from hand speed and breathing here, never from ground contacts. Sore is expected. Sharp or pulling is not, and it ends the round.'},
  fri:{abbr:'FRI',weapons:'all',title:'Defense & Counters',type:'technical',intensity:2,warm:'upperk',cool:'upper',lift:'Lift: Upper B + forearms, including the landmine rotation.',focus:'Read and punish. Sharp hands, quiet legs.',flag:'Tomorrow is the overreach day: trap bar jumps and a heavy squat. Keep the legs out of tonight. Checks and pivots yes, kick volume no. The landmine rotation you did this morning is the same chain as the check hook, so go spend it.'},
  sat:{abbr:'SAT',weapons:'hands',title:'Hands & Core',type:'hard',intensity:5,warm:'upper',cool:'upper',lift:'Lift: Lower Quad + abs. Squat, Bulgarians, calves.',focus:'Hands only. The hard part of today already happened this morning.',flag:'Day six of seven and the biggest total load of the week. Six hours after the lift if you can, four minimum. One core station only, the lift already gave you one. Flat feet, short steps, no bounding. Finishing five minutes early today is a win, not a miss.'},
  sun:{abbr:'SUN',weapons:'all',title:'Restore',type:'recovery',intensity:1,warm:null,cool:null,lift:'No lift today.',focus:'Form, mobility, film. No power, no pace, no exceptions.',flag:'This day exists to make Monday\u2019s kicks possible. Spend the mobility on ankles and hip flexors first, they cap your kick pivot. Hamstrings long and easy only, you are 24 hours off a heavy squat, do not go hunting for a stretch.'}
};
...
{n:1,phase:'Phase 1 · Foundation',theme:'The Tools',note:'One strike at a time. You are cutting the groove, not testing it. Slow and correct beats fast and sloppy every single rep.',
 d:{
  ...
  thu:{tm:{work:180,rest:60,rounds:5},
   t:[['Rotate every 30 sec. 1:00 rest between rounds.'],['30s 1-2 nonstop','max clean output'],['30s push-ups'],['30s fast shadow'],['30s mountain climbers'],['30s straight punch-out','count 90'],['30s 1-2-3 shadow']],
...
{n:2,phase:'Phase 1 · Foundation',theme:'Sharpening',note:'Same tools, more of them, and the body opens up. Volume climbs about 20%. Add the calf kick and the switch kick.',
 d:{
  ...
  thu:{tm:{work:180,rest:60,rounds:5},
   t:[['Rotate every 30 sec. 1:00 rest.'],['30s 1-2-3 nonstop'],['30s push-ups'],['30s shadow with level changes'],['30s mountain climbers'],['30s body-head hands','change levels with the knees, not the waist'],['30s 1-2-3-2']],
  ...
  sat:{tm:{work:180,rest:60,rounds:5},
   t:[['Rotate every 30 sec, hands only. 1:00 rest.'],['30s 1-2-3 max'],['30s push-ups'],['30s body-head hands'],['30s slip counter'],['30s max straights, count 90'],['30s 1-6-3 full speed']],
   r:[['Finisher, 3 min','one station only, the lift already gave you two'],['Bicycles 3 x 40']]},
  sun:{tm:null,
   t:[['Shadow 40-50%, 15 min','the new stuff: calf kick, switch kick, 1-6-3'],['Mobility 15 min','in order, long exhales, no bouncing: 90/90 x10 then 60 sec each side, couch stretch 90 sec each side, frog rocks x10 then 60 sec, elephant walks x10, cossack squats x8 each, pigeon 60 sec each'],['Film 10 min','calf and body kicks. Gaethje low, Barboza body.','Gaethje calf kick Barboza body kick breakdown']],
   r:[]}
 }},
...
{n:3,phase:'Phase 2 · Connection',theme:'Chains',note:'Strikes stop being singles. Hand to kick to hand, and back. Long chains, both stances. This is where it starts to feel like striking instead of drilling.',
 d:{
  ...
  thu:{tm:{work:180,rest:60,rounds:5},
   t:[['Rotate every 30 sec. 1:00 rest.'],['30s five-punch chains'],['30s push-ups'],['30s shadow, level changes mixed in'],['30s mountain climbers'],['30s level change into 1-2','knees bend, hands never touch the floor'],['30s body-head chains']],
  ...
  sat:{tm:{work:180,rest:60,rounds:5},
   t:[['Rotate every 30 sec, hands only. 1:00 rest.'],['30s five-punch chains max'],['30s push-up variations'],['30s body-head'],['30s catch-counter'],['30s max straights, count 90'],['30s free chains']],
   r:[['Finisher, 3 min','one station only, the lift already gave you two'],['Bicycles 3 x 40']]},
  sun:{tm:null,
...
/* ================= WEEK 4 ================= */
{n:4,phase:'Phase 2 · Connection',theme:'Setups',note:'Nothing lands cold. Every strike is bought with a feint, a level change, or a hand that opens the leg. The bag lands at the end of next week, and that is fine: setups are the one thing that costs nothing without one. Sell every feint in the air and the bag will only make them louder.',
 d:{
  mon:{tm:{work:180,rest:60,rounds:3},
   t:[['Hand feint into leg kick x12 each','flash the jab, kick low'],['Teep feint into leg kick x12 each'],['Body kick then head kick, same side x10 each','body first every time, the head kick is the second beat'],['Question mark kick x12 each'],['Level change into head kick x10 each','last drill on purpose, the base foot is warm now']],
  ...
  thu:{tm:{work:180,rest:60,rounds:5},
   t:[['Rotate every 30 sec. 1:00 rest.'],['30s feint-combo nonstop'],['30s push-ups'],['30s shadow with feints'],['30s mountain climbers'],['30s slip and counter, fast'],['30s body-head chains']],
   r:[['Finisher, 5 min'],['Plank 3 x 60 sec'],['Hollow hold 3 x 40 sec'],['Leg raises 3 x 20'],['Russian twists 3 x 40']]},
  fri:{tm:{work:150,rest:60,rounds:3},
...
/* ================= WEEK 5 ================= */
{n:5,phase:'Phase 3 · Reaction',theme:'Counters',note:'Stop initiating everything. Read, react, punish. THE BAG LANDS FRIDAY: Monday to Thursday is still air work, then wraps and gloves go on and the last rounds of the week are the first contact your hands and shins have ever had. Fifty percent, both. NO BAG YET: shadow every Rounds block and treat the bag weeks as postponed, not skipped.',
 d:{
  mon:{tm:{work:180,rest:60,rounds:3},
  ...
  thu:{tm:{work:180,rest:60,rounds:5},
   t:[['Rotate every 30 sec. 1:00 rest.'],['30s counter chains'],['30s push-ups'],['30s slip-counter shadow'],['30s mountain climbers'],['30s uppercuts up the middle'],['30s free counters']],
  ...
  sun:{tm:null,
   t:[['Shadow 40-50%, 15 min','every counter, slow'],['Neck isometrics','forehead, both temples, then the back of the head. Press into your own palm, 10 sec each, breathe'],['Mobility 15 min','in order, long exhales, no bouncing: 90/90 x10 then 60 sec each side, couch stretch 90 sec each side, frog rocks x10 then 60 sec, elephant walks x10, cossack squats x8 each, pigeon 60 sec each'],['Film 10 min','counters. Anderson Silva, the beat before it lands, and watch the timing only, do not copy the hands-down guard. Then run a Paddy Pimblett round with the sound off and count how often his chin sits high and straight up the middle. His grappling bails him out. On the feet you will not have that excuse, and this is the exact habit these five weeks are built to kill.','Anderson Silva counter timing breakdown']],
   r:[]}
 }},

/* ================= WEEK 6 ================= */
{n:6,phase:'Phase 3 · Reaction',theme:'Timing & Angles',note:'Same strikes, better real estate. Stop trading in front of people. Hit from where they are not looking, and never end a combo where you started it. FIRST FULL WEEK ON THE BAG: every Rounds block runs on it, wraps and gloves every time, hands and kicks held at 50 percent all week while the wrists and shins learn what contact is. NO BAG YET: run it all in the air, the angles are the lesson and they do not need a target.',
 d:{
  mon:{tm:{work:180,rest:60,rounds:3},
   t:[['Range check 2 min','stand where the shin lands, not the foot, and step back out to that range after every strike'],['Step off-angle, then kick x12 each'],['Kick, pivot, kick x10 each'],['Switch stance, kick, switch back x12'],['Southpaw leg kick x12','switch and kick from the other side'],['Teep to create the angle x12']],
  ...
  wed:{tm:{work:180,rest:60,rounds:3},
   t:[['Wrap and glove up','every bag round, no exceptions. A locked straight wrist is the whole job'],['Pivot drills x20 each way'],['1-2, pivot left, 1-2 x12'],['Jab, step off-line, cross x12'],['Cut the angle, hook x12'],['In-out-in x15','step in, out, back in with a shot']],
   r:[['R1 footwork only','no punches, just move'],['R2','only punch off an angle'],['R3','free, never square up']]},
  thu:{tm:{work:180,rest:60,rounds:5},
   t:[['Rotate every 30 sec. 1:00 rest.'],['30s combo-angle-combo'],['30s push-ups'],['30s shadow, constant movement'],['30s mountain climbers'],['30s combo, then angle out'],['30s free with angles']],
   r:[['Finisher, 5 min'],['Plank 3 x 60 sec'],['Hollow hold 3 x 45 sec'],['Leg raises 3 x 22'],['Russian twists 3 x 45']]},
  fri:{tm:{work:180,rest:60,rounds:3},
  ...
  sat:{tm:{work:180,rest:60,rounds:5},
   ...
   r:[['Finisher, 3 min','one station only, the lift already gave you two'],['Bicycles 3 x 50']]},
  sun:{tm:null,
   t:[['Shadow 40-50%, 15 min','angles only, slow'],['Neck isometrics','forehead, both temples, then the back of the head. Press into your own palm, 10 sec each, breathe'],['Mobility 15 min','in order, long exhales, no bouncing: 90/90 x10 then 60 sec each side, couch stretch 90 sec each side, frog rocks x10 then 60 sec, elephant walks x10, cossack squats x8 each, pigeon 60 sec each'],['Film 10 min','feet only. Stephen Wonderboy Thompson, ignore his hands and his stance entirely. Watch how he never ends where he started.','Wonderboy Thompson footwork breakdown']],
   r:[]}
 }},

/* ================= WEEK 7 ================= */
{n:7,phase:'Phase 4 · Power',theme:'Heavy Hands',note:'First full bag phase. Power is a skill air cannot teach: ground force through rotation into a real object. Sit down on every shot, kick through the bag, kicks at 70% while the shins keep adapting. NO BAG YET: run it as shadow, chase snap and full retraction instead of impact, and treat Power as postponed rather than skipped.',
 d:{
  mon:{tm:{work:180,rest:60,rounds:4},
   t:[['Range check 2 min','stand where the shin lands, not the foot'],['Leg kick x15 each','70%, kick through it, bag folds around the shin'],['Body kick x15 each','hip turns all the way over on contact'],['Power teep x15 each','rock it straight back'],['Switch kick x12 each','the bag tells you if the hip loaded']],
   r:[['R1 kicks only','through the bag, never at it'],['R2 kick-hand-kick','leg kick, 1-2, body kick'],['R3 free kicks','step back to range after every strike'],['R4 free','every kick you own, fight pace, land balanced']]},
  tue:{tm:{work:180,rest:60,rounds:4},
   t:[['1-2 at 70% x20','sit down, make the bag jump'],['1-2-3 x15','hook meets the bag swinging back'],['Hand-kick-hand x12 each','1-2, leg kick, 2'],['3-2 in close x12','short and heavy'],['Chain into low kick x10','1-2-3-2, finish on the leg']],
  ...
  wed:{tm:{work:180,rest:60,rounds:4},
   ...
   r:[['R1 jab only','real contact, real snap'],['R2 body only','invest downstairs all round'],['R3 all hands 70%','light feet, you jumped this morning'],['R4 free hands','chain, angle, chain, no coasting']]},
  thu:{tm:{work:180,rest:60,rounds:5},
   t:[['Rotate every 30 sec on the bag. 1:00 rest.'],['30s max output'],['30s push-ups'],['30s body shots only'],['30s mountain climbers'],['30s 1-2 on the bag, max hands'],['30s free hands']],
  ...
  sat:{tm:{work:180,rest:60,rounds:5},
   ...
   r:[['Finisher, 3 min','one station only, the lift already gave you two'],['Bicycles 3 x 50']]},
  sun:{tm:null,
   t:[['Shadow 40-50%, 15 min','no bag today, groove the clean form back in'],['Neck isometrics','forehead, both temples, then the back of the head. Press into your own palm, 10 sec each, breathe'],['Mobility 15 min','shins and hips took new impact, take all of it'],['Film 10 min','a power puncher. Ilia Topuria or Robert Whittaker, watch the rear heel and the knees. Skip the heavyweights, that power is not mechanics you can copy.','Ilia Topuria power punching mechanics']],
   r:[]}
 }},

/* ================= WEEK 8 ================= */
{n:8,phase:'Phase 4 · Power',theme:'Pressure & Pace',note:'The bag swings, and the swing is an opponent moving. Hit it coming in, chase it going out, hold fight pace throughout. Hardest week of the camp. Kicks at 85 percent, full power comes next week. NO BAG YET: shadow the same drills against an imagined opponent who advances and retreats, and keep the pace honest since pace is the real lesson here.',
 d:{
  mon:{tm:{work:180,rest:60,rounds:4},
   ...
   r:[['R1 volume','kick output, feet never stop'],['R2 the swing','time every kick to it coming in'],['R3 free','kicks and knees at fight pace'],['R4 free','every kick you own, fight pace, land balanced']]},
  tue:{tm:{work:180,rest:60,rounds:4},
  ...
  wed:{tm:{work:180,rest:60,rounds:4},
   ...
   r:[['R1 timing','only punch when it comes to you'],['R2 in-and-out','never flat, never square'],['R3','free, last 30 sec is a punch-out'],['R4 free hands','chain, angle, chain, no coasting']]},
  thu:{tm:{work:180,rest:60,rounds:5},
   t:[['Rotate every 30 sec on the bag. 1:00 rest. Hardest session of the camp.'],['30s all-out'],['30s push-ups'],['30s body shots'],['30s mountain climbers'],['30s burst of six, then reset'],['30s punch-out, straights only']],
   r:[['Finisher, 5 min'],['Plank 3 x 60 sec'],['Hollow hold 3 x 45 sec'],['Leg raises 3 x 25'],['Russian twists 3 x 50']]},
  fri:{tm:{work:180,rest:60,rounds:4},
   ...
   r:[['R1','pure reaction, no leads'],['R2','counter, angle, re-enter'],['R3','free, make it miss then make it pay'],['R4 free','make it miss, make it pay, angle out']]},
  sat:{tm:{work:180,rest:60,rounds:5},
   t:[['Rotate every 30 sec, hands on the bag. 1:00 rest.'],['30s punch-out'],['30s push-up variations'],['30s hit the advance'],['30s slip-counter'],['30s uppercuts in close'],['30s fight pace free']],
   r:[['Finisher, 3 min','one station only, the lift already gave you two'],['Bicycles 3 x 50']]},
  sun:{tm:null,
   t:[['Shadow 40-50%, 15 min','slow everything back down'],['Neck isometrics','forehead, both temples, then the back of the head. Press into your own palm, 10 sec each, breathe'],['Mobility 15 min','biggest week yet, take every minute'],['Film 10 min','cage cutting. Khabib or Merab, cutting not chasing. Then Paddy Pimblett for the volume version of the same idea: he walks people down and buries them in output, and the grappling threat behind it is why the pressure works. That blend is the one you already have the base for.','Khabib Merab cutting the cage breakdown']],
   r:[]}
 }},
...
{n:9,phase:'Phase 5 · Creation',theme:'Rhythm',note:'A readable rhythm is how you get hit. Fast, slow, pause, burst, on the bag now. NO BAG YET: rhythm is the one thing that costs you nothing without one, so run every bit of it in the air. Same strikes on a beat nobody can count. This is the week it starts being fun.',
 d:{
  mon:{tm:{work:180,rest:60,rounds:4},
   ...
   r:[['R1','change speed every combo'],['R2','pause, then explode'],['R3','free, no rhythm they can read'],['R4 free','every kick you own, fight pace, land balanced']]},
  tue:{tm:{work:180,rest:60,rounds:4},
  ...
  wed:{tm:{work:180,rest:60,rounds:4},
   ...
   r:[['R1','vary the jab rhythm all round'],['R2','bursts only'],['R3','free rhythm'],['R4 free hands','chain, angle, chain, no coasting']]},
  thu:{tm:{work:180,rest:60,rounds:5},
   t:[['Rotate every 30 sec. 1:00 rest.'],['30s burst, reset, burst'],['30s push-ups'],['30s broken-rhythm shadow'],['30s mountain climbers'],['30s counter on the half beat'],['30s free rhythm']],
   r:[['Finisher, 5 min'],['Plank 3 x 60 sec'],['Hollow hold 3 x 45 sec'],['Leg raises 3 x 25'],['Russian twists 3 x 50']]},
  fri:{tm:{work:180,rest:60,rounds:4},
   ...
   r:[['R1','bait and counter'],['R2','counter on the half beat'],['R3','free, make them commit then punish'],['R4 free','make it miss, make it pay, angle out']]},
  sat:{tm:{work:180,rest:60,rounds:5},
...
{n:10,phase:'Phase 5 · Creation',theme:'Free',note:'No prescribed combos. Everything you built, unscripted, on the bag and in the air. NO BAG YET: all of it in the air then, and it still counts. Run the 5-point checkpoint Sunday, then book the trial class.',
 d:{
  mon:{tm:{work:180,rest:60,rounds:4},
   ...
   r:[['R1','orthodox free'],['R2','southpaw free'],['R3','switch freely, mid-combo'],['R4 free','every kick you own, fight pace, land balanced']]},
  tue:{tm:{work:180,rest:60,rounds:4},
  ...
  wed:{tm:{work:180,rest:60,rounds:4},
   ...
   r:[['R1','free, pretend it is round 1'],['R2','free, pretend you are down'],['R3','free, pretend you are ahead and managing'],['R4 free hands','chain, angle, chain, no coasting']]},
  thu:{tm:{work:180,rest:60,rounds:5},
   t:[['Rotate every 30 sec. 1:00 rest.'],['30s free max output'],['30s push-ups'],['30s free shadow'],['30s mountain climbers'],['30s body-head, dig downstairs'],['30s free, whatever comes']],
   r:[['Finisher, 5 min'],['Plank 3 x 60 sec'],['Hollow hold 3 x 45 sec'],['Leg raises 3 x 25'],['Russian twists 3 x 50']]},
  fri:{tm:{work:180,rest:60,rounds:4},
   ...
   r:[['R1','pure reaction, no leads'],['R2','defend, counter, angle, repeat'],['R3','free'],['R4 free','make it miss, make it pay, angle out']]},
  sat:{tm:{work:180,rest:60,rounds:5},
   t:[['Rotate every 30 sec, hands only. 1:00 rest.'],['30s free max'],['30s push-up variations'],['30s free hands'],['30s free counters'],['30s free, always off angle'],['30s fight sim pace']],
   r:[['Finisher, 3 min','one station only, the lift already gave you two'],['Bicycles 3 x 50']]},
  sun:{tm:null,
   t:[['Film one full round','then run the checkpoint below'],['Neck isometrics','forehead, both temples, then the back of the head. Press into your own palm, 10 sec each, breathe'],['1. Hands','return to guard every time, zero drops'],['2. Feet','never cross, never square, never flat'],['3. Pivot','every kick pivots, every landing balanced'],['4. Flow','1-2-3 is one motion, not three decisions'],['5. Gas','last round looks like the first']],
...
{cat:'Kicks',moves:[
 ...
 {name:'Question Mark Kick',tag:'Fakes low, lands high.',steps:['Chamber exactly like a leg kick so they drop the elbow to check.','Snap the knee up and over the top into the head instead.'],cue:'It only works if your leg kick is real, and you have two weeks of low kicks in the bank. Throw it late in the session, never first, and only as high as the hips already go.',vid:'question mark kick technique'},
...
{cat:'Bag Work',moves:[
 ...
 {name:'Shin Conditioning',tag:'The bag is the method.',steps:['First contact is the end of week 5, at 50 percent. Week 6 the whole week at 50. Week 7 at 70. Week 8 at 85. Week 9 on, full power.','The adaptation is progressive impact, nothing else.'],cue:'Sore shins early are normal. Bottle-rolling tricks are noise, the bag is the conditioning.',vid:'shin conditioning muay thai truth'},
